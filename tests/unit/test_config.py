...
class TestConfigDefaults:
    ...
    def test_embedding_provider_default(self, monkeypatch):
        """SKILLPORT_EMBEDDING_PROVIDER defaults to 'local'."""
...
class TestConfigProviderValidation:
    ...
    def test_openai_compatible_requires_base_url(self, monkeypatch):
        monkeypatch.setenv("SKILLPORT_EMBEDDING_PROVIDER", "openai_compatible")
        monkeypatch.delenv("SKILLPORT_EMBEDDING_BASE_URL", raising=False)
        monkeypatch.setenv("SKILLPORT_EMBEDDING_MODEL", "test-model")
        monkeypatch.setenv("SKILLPORT_EMBEDDING_API_KEY", "sk-test-key")
        with pytest.raises(ValueError, match="SKILLPORT_EMBEDDING_BASE_URL"):
            Config()

    def test_openai_compatible_requires_model(self, monkeypatch):
        monkeypatch.setenv("SKILLPORT_EMBEDDING_PROVIDER", "openai_compatible")
        monkeypatch.setenv("SKILLPORT_EMBEDDING_BASE_URL", "http://localhost:8000/v1")
        monkeypatch.delenv("SKILLPORT_EMBEDDING_MODEL", raising=False)
        monkeypatch.setenv("SKILLPORT_EMBEDDING_API_KEY", "sk-test-key")
        with pytest.raises(ValueError, match="SKILLPORT_EMBEDDING_MODEL"):
            Config()

    def test_openai_compatible_requires_key(self, monkeypatch):
        monkeypatch.setenv("SKILLPORT_EMBEDDING_PROVIDER", "openai_compatible")
        monkeypatch.setenv("SKILLPORT_EMBEDDING_BASE_URL", "http://localhost:8000/v1")
        monkeypatch.setenv("SKILLPORT_EMBEDDING_MODEL", "test-model")
        monkeypatch.delenv("SKILLPORT_EMBEDDING_API_KEY", raising=False)
        with pytest.raises(ValueError, match="SKILLPORT_EMBEDDING_API_KEY"):
            Config()

    def test_local_provider_allows_no_key(self, monkeypatch):
        monkeypatch.setenv("SKILLPORT_EMBEDDING_PROVIDER", "local")
        monkeypatch.setenv("SKILLPORT_EMBEDDING_BASE_URL", "http://localhost:8000/v1")
        monkeypatch.setenv("SKILLPORT_EMBEDDING_MODEL", "test-model")
        monkeypatch.delenv("SKILLPORT_EMBEDDING_API_KEY", raising=False)
        cfg = Config()
        assert cfg.embedding_provider == "local"
        assert cfg.embedding_base_url == "http://localhost:8000/v1"
