...
Vector search is optional. By default, SkillPort uses local embeddings (Ollama-style endpoints). If local embeddings are unavailable, SkillPort automatically falls back to full-text search. Set `SKILLPORT_EMBEDDING_PROVIDER=none` to disable embeddings and use full-text search only.

| Variable | Description | Default |
|----------|-------------|---------|
| `SKILLPORT_EMBEDDING_PROVIDER` | Embedding provider (`none`, `openai`, `openai_compatible`, `dashscope`, `zhipu`, `baidu`, `tencent`, `local`) | `local` |
| `OPENAI_API_KEY` | OpenAI API key (required when provider is `openai`) | none |
| `OPENAI_EMBEDDING_MODEL` | OpenAI embedding model | `text-embedding-3-small` |
| `SKILLPORT_EMBEDDING_BASE_URL` | Base URL for OpenAI-compatible or local providers | `http://localhost:11434/v1` |
| `SKILLPORT_EMBEDDING_MODEL` | Model name for OpenAI-compatible or local providers | `bge-m3` |
| `SKILLPORT_EMBEDDING_API_KEY` | API key for OpenAI-compatible providers (not required for `local`) | none |
| `DASHSCOPE_API_KEY` | Alias for `SKILLPORT_EMBEDDING_API_KEY` when using DashScope | none |

**Examples:**

```bash
# OpenAI-compatible provider (domestic vendor with OpenAI-compatible endpoint)
export SKILLPORT_EMBEDDING_PROVIDER=openai_compatible
export SKILLPORT_EMBEDDING_BASE_URL=https://example.vendor.com/v1
export SKILLPORT_EMBEDDING_MODEL=vendor-embedding-model
export SKILLPORT_EMBEDDING_API_KEY=sk-...
```

```bash
# Local embedding server (no API key required)
export SKILLPORT_EMBEDDING_PROVIDER=local
export SKILLPORT_EMBEDDING_BASE_URL=http://localhost:8000/v1
export SKILLPORT_EMBEDDING_MODEL=bge-m3
```
