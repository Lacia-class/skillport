...
- [Commands](#commands)
  ...
  - [set-embedding](#skillport-set-embedding) - Set embedding provider
...
### skillport set-embedding

Set embedding provider and write env settings (defaults to `.env` in current directory).

```bash
skillport set-embedding <provider> [options]
```
...
| Option | Description | Default |
|--------|-------------|---------|
| `--base-url` | Embedding base URL | provider-specific |
| `--model` | Embedding model name | provider-specific |
| `--api-key` | API key to store | none |
| `--env-file` | Env file to update | `.env` |

#### Examples

```bash
# Local (Ollama)
skillport set-embedding local

# DashScope (uses default base URL and model)
skillport set-embedding dashscope

# OpenAI
skillport set-embedding openai --api-key sk-... --model text-embedding-3-small

# OpenAI-compatible server
skillport set-embedding openai_compatible --base-url http://localhost:8000/v1 --model bge-m3

# Disable embeddings
skillport set-embedding none
```

> **Note**: The `.env` file is loaded from the current working directory. Run the command in the directory where you start `skillport`.
