...
class Config(BaseSettings):
    ...
    embedding_model: str | None = Field(
        default=None,
        description="Embedding model for OpenAI-compatible providers",
    )
    # Search
    ...
    def validate_provider_keys(self):
        ...
        if self.embedding_provider in openai_compatible:
            if self.embedding_provider != "local":
                if not self.embedding_base_url:
                    raise ValueError(
                        "SKILLPORT_EMBEDDING_BASE_URL is required when embedding_provider is "
                        f"'{self.embedding_provider}'"
                    )
                if not self.embedding_model:
                    raise ValueError(
                        "SKILLPORT_EMBEDDING_MODEL is required when embedding_provider is "
                        f"'{self.embedding_provider}'"
                    )
                if not self.embedding_api_key:
                    raise ValueError(
                        "SKILLPORT_EMBEDDING_API_KEY is required when embedding_provider is "
                        f"'{self.embedding_provider}'"
                    )
        return self
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.embedding_provider == "local":
            if self.embedding_base_url is None:
                object.__setattr__(self, "embedding_base_url", "http://localhost:11434/v1")
            if self.embedding_model is None:
                object.__setattr__(self, "embedding_model", "bge-m3")
