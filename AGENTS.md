...
* **Safe defaults**: Default `EMBEDDING_PROVIDER=local` with Ollama defaults; when enabling external providers, fail fast on missing keys.
...
*   **Run Server (Manual)**:
    ```bash
    SKILLPORT_SKILLS_DIR=.agent/skills SKILLPORT_EMBEDDING_PROVIDER=local uv run skillport
    ```
